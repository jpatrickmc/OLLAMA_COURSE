...
# pip install streamlit ollama openai-whisper opencv-python pydub numpy
...
#
# How to run:
# streamlit run video_to_text.py
...
def save_uploaded_video(uploaded_file):
    ...
    # This avoids the pydub/audioop compatibility issue on Python 3.13+.
    subprocess.run(
        ["ffmpeg", "-i", temp_video_path, "-q:a", "9", "-n", audio_path],
        capture_output=True,
        check=True
    )
...
if uploaded_file is not None:
    ...
    if button:
        if prompt:
            ...
            # 5. Generate Answer
            response = ollama.generate(model='llama2', prompt=combined_prompt)
